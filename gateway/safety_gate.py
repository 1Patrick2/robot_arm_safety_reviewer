...
def review_only(
    ...
    *,
    ...
    try:
        ...
        result = evaluate_joint_command(scene, command, backend=backend)
        review_backend = _backend_metadata(backend)
...
def _backend_metadata(backend) -> dict:
    metadata = {"name": getattr(backend, "name", "unknown")}
    review_metadata = getattr(backend, "last_review_metadata", None)
    if isinstance(review_metadata, dict):
        metadata.update(review_metadata)
    elif metadata["name"] == "mock":
        metadata["model_version"] = "mock_geometry_v1"
        metadata["collision_method"] = "segment_sphere_clearance"
    return metadata
